from sqlalchemy import String, ForeignKey, Integer, DateTime, CheckConstraint, Index,Text
...
class StockMovement(Base):
    ...
    __table_args__ = (  
        ...
        CheckConstraint("reference_type IN ('purchase_order', 'sale', 'adjustment', 'transfer')", name="ck_reference_type"),
    )
...
class StockAdjustment(Base):
    ...
    # Relationships
    product: Mapped["Product"] = relationship("Product", back_populates="stock_adjustments")
    creator: Mapped["User"] = relationship("User", back_populates="stock_adjustments")
    
    __table_args__ = (
        CheckConstraint("adjustment_type IN ('damaged', 'expired', 'returned', 'found', 'theft', 'internal_use')", name="ck_adjustment_type"),
    )
