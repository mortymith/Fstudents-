...
class AsyncDatabase:
    ...
    def __init__(
        ...
        *,
        ...
        self._engine_options = {
            ...
            "connect_args": {
                "command_timeout": 60,
                "server_settings": {"application_name": "async_db_client"},
            },
        }
        ...
        logger.debug(
            f"AsyncDatabase instance created for: {self._mask_url(database_url)}"
        )
        ...
        async with self._initialization_lock:
            ...
            try:
                ...
                self._engine = create_async_engine(
                    self._database_url, **self._engine_options
                )

                # Create session factory
                self._session_factory = async_sessionmaker(
                    self._engine, **self._session_options
                )
    ...
    async def session(self) -> AsyncGenerator[AsyncSession, None]:
        ...
        if not self._initialized:
            raise RuntimeError(
                "AsyncDatabase not initialized. Call initialize() first."
            )
...
class DatabasePool:
    ...
    async def add_database(
        self, name: str, database_url: str, **kwargs
    ) -> AsyncDatabase:
        """
...
async def create_database_client(database_url: str, **kwargs) -> AsyncDatabase:
    """
...
async def create_database_pool(databases: dict[str, str], **kwargs) -> DatabasePool:
    """
