...
class ProductInventoryRepository:
    ...
    async def get_all(self, session: AsyncSession, *, skip: int = 0, limit: int = 100) -> List[ProductInventory]:
        """
    ...
    async def update(self, session: AsyncSession, id: Any, **kwargs) -> Optional[ProductInventory]:
        """
    ...
    async def get_by_product_id(self, session: AsyncSession, product_id: int) -> Optional[ProductInventory]:
        """
    ...
    async def filter_by_available_quantity(self, session: AsyncSession, min_quantity: int = 0, max_quantity: Optional[int] = None, *, skip: int = 0, limit: int = 100) -> List[ProductInventory]:
        """
        ...
        try:
            ...
            stmt = select(ProductInventory).where(and_(*conditions)).offset(skip).limit(limit)
            result = await session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
    ...
    async def filter_by_on_hand_quantity(self, session: AsyncSession, min_quantity: int = 0, max_quantity: Optional[int] = None, *, skip: int = 0, limit: int = 100) -> List[ProductInventory]:
        """
    ...
    async def get_low_stock_inventory(self, session: AsyncSession, threshold: int = 10, *, skip: int = 0, limit: int = 100) -> List[ProductInventory]:
        """
    ...
    async def get_out_of_stock_inventory(self, session: AsyncSession, *, skip: int = 0, limit: int = 100) -> List[ProductInventory]:
        """
    ...
    async def get_or_create(self, session: AsyncSession, defaults: Optional[dict] = None, **kwargs) -> Tuple[ProductInventory, bool]:
        """
    ...
    async def update_or_create(self, session: AsyncSession, criteria: dict, updates: dict) -> Tuple[ProductInventory, bool]:
        """
        ...
        try:
            if 'product_id' in criteria:
                inventory = await self.get_by_product_id(session, criteria['product_id'])
            else:
                # Try to find by any criteria
                conditions = [getattr(ProductInventory, key) == value for key, value in criteria.items()]
    ...
    async def get_with_product(self, session: AsyncSession, id: Any) -> Optional[ProductInventory]:
        """
        ...
        """
        try:
            stmt = select(ProductInventory).options(selectinload(ProductInventory.product)).where(ProductInventory.id == id)
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
        except Exception as e:
    ...
    async def update_quantity(self, session: AsyncSession, product_id: int, quantity_change: int, commit: bool = True) -> Optional[ProductInventory]:
        """
        ...
        try:
            ...
            inventory.quantity_available = inventory.quantity_on_hand - inventory.quantity_committed
    ...
    async def update_committed_quantity(self, session: AsyncSession, product_id: int, quantity_change: int, commit: bool = True) -> Optional[ProductInventory]:
        """
        ...
        try:
            ...
            if commit:
                await session.rollback()
            logger.error(f"Error updating committed quantity for product {product_id}: {e}")
            return None
    
    async def update_restock_date(self, session: AsyncSession, product_id: int) -> Optional[ProductInventory]:
        """
    ...
    async def update_count_date(self, session: AsyncSession, product_id: int) -> Optional[ProductInventory]:
        """
