...
class PurchaseOrderItemRepository:
    ...
    async def get_all(self, session: AsyncSession, *, skip: int = 0, limit: int = 100) -> List[PurchaseOrderItem]:
        """
    ...
    async def update(self, session: AsyncSession, id: Any, **kwargs) -> Optional[PurchaseOrderItem]:
        """
    ...
    async def get_by_purchase_order_and_product(self, session: AsyncSession, purchase_order_id: int, product_id: int) -> Optional[PurchaseOrderItem]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting purchase order item by PO {purchase_order_id} and product {product_id}: {e}")
            return None
    
    async def filter_by_purchase_order(self, session: AsyncSession, purchase_order_id: int, *, skip: int = 0, limit: int = 100) -> List[PurchaseOrderItem]:
        """
        ...
        """
        try:
            stmt = select(PurchaseOrderItem).where(PurchaseOrderItem.purchase_order_id == purchase_order_id).offset(skip).limit(limit)
            result = await session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
            logger.error(f"Error filtering purchase order items by purchase order {purchase_order_id}: {e}")
            return []
    
    async def filter_by_product(self, session: AsyncSession, product_id: int, *, skip: int = 0, limit: int = 100) -> List[PurchaseOrderItem]:
        """
        ...
        """
        try:
            stmt = select(PurchaseOrderItem).where(PurchaseOrderItem.product_id == product_id).offset(skip).limit(limit)
            result = await session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
            logger.error(f"Error filtering purchase order items by product {product_id}: {e}")
            return []
    
    async def filter_by_quantity_ordered(self, session: AsyncSession, min_quantity: int = 0, max_quantity: Optional[int] = None, *, skip: int = 0, limit: int = 100) -> List[PurchaseOrderItem]:
        """
        ...
        try:
            ...
            stmt = select(PurchaseOrderItem).where(and_(*conditions)).offset(skip).limit(limit)
            result = await session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
            logger.error(f"Error filtering purchase order items by ordered quantity: {e}")
            return []
    
    async def filter_by_unit_cost(self, session: AsyncSession, min_cost: Decimal, max_cost: Decimal, *, skip: int = 0, limit: int = 100) -> List[PurchaseOrderItem]:
        """
        ...
        try:
            stmt = (
                ...
                .where(and_(PurchaseOrderItem.unit_cost >= min_cost, PurchaseOrderItem.unit_cost <= max_cost))
                .offset(skip)
                .limit(limit)
            )
            result = await session.execute(stmt)
    ...
    async def get_or_create(self, session: AsyncSession, defaults: Optional[dict] = None, **kwargs) -> Tuple[PurchaseOrderItem, bool]:
        """
    ...
    async def update_or_create(self, session: AsyncSession, criteria: dict, updates: dict) -> Tuple[PurchaseOrderItem, bool]:
        """
        ...
        try:
            if 'purchase_order_id' in criteria and 'product_id' in criteria:
                ...
                conditions = [getattr(PurchaseOrderItem, key) == value for key, value in criteria.items()]
            ...
            if item:
                ...
                if 'line_total' not in item_data and 'quantity_ordered' in item_data and 'unit_cost' in item_data:
                    item_data['line_total'] = item_data['quantity_ordered'] * item_data['unit_cost']
    ...
    async def get_with_purchase_order(self, session: AsyncSession, id: Any) -> Optional[PurchaseOrderItem]:
        """
        ...
        """
        try:
            stmt = select(PurchaseOrderItem).options(selectinload(PurchaseOrderItem.purchase_order)).where(PurchaseOrderItem.id == id)
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error getting purchase order item with purchase order {id}: {e}")
            return None
    
    async def get_with_product(self, session: AsyncSession, id: Any) -> Optional[PurchaseOrderItem]:
        """
        ...
        """
        try:
            stmt = select(PurchaseOrderItem).options(selectinload(PurchaseOrderItem.product)).where(PurchaseOrderItem.id == id)
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
        except Exception as e:
    ...
    async def update_received_quantity(self, session: AsyncSession, id: int, quantity_received: int) -> Optional[PurchaseOrderItem]:
        """
        ...
        try:
            ...
            await session.rollback()
            logger.error(f"Error updating received quantity for purchase order item {id}: {e}")
            return None
    
    async def get_partially_received_items(self, session: AsyncSession, *, skip: int = 0, limit: int = 100) -> List[PurchaseOrderItem]:
        """
    ...
    async def get_fully_received_items(self, session: AsyncSession, *, skip: int = 0, limit: int = 100) -> List[PurchaseOrderItem]:
        """
        ...
        try:
            stmt = (
                select(PurchaseOrderItem)
                .where(PurchaseOrderItem.quantity_received == PurchaseOrderItem.quantity_ordered)
                .offset(skip)
                .limit(limit)
            )
            result = await session.execute(stmt)
    ...
    async def get_product_purchase_history(self, session: AsyncSession, product_id: int, *, skip: int = 0, limit: int = 100) -> List[PurchaseOrderItem]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting product purchase history for product {product_id}: {e}")
            return []
    
    async def calculate_product_total_spend(self, session: AsyncSession, product_id: int) -> Decimal:
        """
        ...
        """
        try:
            stmt = select(func.sum(PurchaseOrderItem.line_total)).where(PurchaseOrderItem.product_id == product_id)
            result = await session.execute(stmt)
    ...
    async def get_items_with_product_details(self, session: AsyncSession, purchase_order_id: int) -> List[PurchaseOrderItem]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting items with product details for PO {purchase_order_id}: {e}")
            return []