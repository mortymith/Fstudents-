...
class UserRepository:
    ...
    async def update_or_create(
        ...
        try:
            if "email" in criteria:
                ...
                conditions = [
                    getattr(User, key) == value for key, value in criteria.items()
                ]
    ...
    async def get_with_purchase_orders(
        ...
        try:
            stmt = (
                select(User)
                .options(selectinload(User.created_purchase_orders))
                .where(User.id == id)
            )
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
    ...
    async def get_with_stock_movements(
        self, session: AsyncSession, id: Any
    ) -> Optional[User]:
        """
        ...
        try:
            stmt = (
                select(User)
                .options(selectinload(User.stock_movements))
                .where(User.id == id)
            )
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
    ...
    async def get_with_stock_adjustments(
        self, session: AsyncSession, id: Any
    ) -> Optional[User]:
        """
        ...
        try:
            stmt = (
                select(User)
                .options(selectinload(User.stock_adjustments))
                .where(User.id == id)
            )
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
    ...
    async def get_users_with_recent_login(
