...
class StockMovementRepository:
    ...
    async def get_all(self, session: AsyncSession, *, skip: int = 0, limit: int = 100) -> List[StockMovement]:
        """
    ...
    async def update(self, session: AsyncSession, id: Any, **kwargs) -> Optional[StockMovement]:
        """
    ...
    async def get_by_product_id(self, session: AsyncSession, product_id: int) -> Optional[StockMovement]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting stock movement by product ID {product_id}: {e}")
            return None
    
    async def filter_by_product(self, session: AsyncSession, product_id: int, *, skip: int = 0, limit: int = 100) -> List[StockMovement]:
        """
        ...
        try:
            ...
            logger.error(f"Error filtering stock movements by product {product_id}: {e}")
            return []
    
    async def filter_by_movement_type(self, session: AsyncSession, movement_type: str, *, skip: int = 0, limit: int = 100) -> List[StockMovement]:
        """
        ...
        """
        try:
            stmt = select(StockMovement).where(StockMovement.movement_type == movement_type).offset(skip).limit(limit)
            result = await session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
            logger.error(f"Error filtering stock movements by type {movement_type}: {e}")
            return []
    
    async def filter_by_creator(self, session: AsyncSession, created_by: int, *, skip: int = 0, limit: int = 100) -> List[StockMovement]:
        """
        ...
        """
        try:
            stmt = select(StockMovement).where(StockMovement.created_by == created_by).offset(skip).limit(limit)
            result = await session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
            logger.error(f"Error filtering stock movements by creator {created_by}: {e}")
            return []
    
    async def filter_by_reference(self, session: AsyncSession, reference_type: str, reference_id: int, *, skip: int = 0, limit: int = 100) -> List[StockMovement]:
        """
        ...
        try:
            ...
            logger.error(f"Error filtering stock movements by reference {reference_type}/{reference_id}: {e}")
            return []
    
    async def filter_by_date_range(self, session: AsyncSession, start_date: datetime, end_date: datetime, *, skip: int = 0, limit: int = 100) -> List[StockMovement]:
        """
        ...
        try:
            stmt = (
                select(StockMovement)
                .where(and_(StockMovement.movement_date >= start_date, StockMovement.movement_date <= end_date))
                .order_by(desc(StockMovement.movement_date))
                .offset(skip)
                .limit(limit)
            )
    ...
    async def filter_by_quantity_change(self, session: AsyncSession, min_change: int, max_change: Optional[int] = None, *, skip: int = 0, limit: int = 100) -> List[StockMovement]:
        """
        ...
        try:
            ...
            stmt = select(StockMovement).where(and_(*conditions)).offset(skip).limit(limit)
            result = await session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
    ...
    async def get_or_create(self, session: AsyncSession, defaults: Optional[dict] = None, **kwargs) -> Tuple[StockMovement, bool]:
        """
    ...
    async def update_or_create(self, session: AsyncSession, criteria: dict, updates: dict) -> Tuple[StockMovement, bool]:
        """
        ...
        try:
            ...
            conditions = [getattr(StockMovement, key) == value for key, value in criteria.items()]
    ...
    async def get_with_product(self, session: AsyncSession, id: Any) -> Optional[StockMovement]:
        """
        ...
        """
        try:
            stmt = select(StockMovement).options(selectinload(StockMovement.product)).where(StockMovement.id == id)
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
        except Exception as e:
    ...
    async def get_with_creator(self, session: AsyncSession, id: Any) -> Optional[StockMovement]:
        """
        ...
        """
        try:
            stmt = select(StockMovement).options(selectinload(StockMovement.creator)).where(StockMovement.id == id)
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
        except Exception as e:
    ...
    async def get_recent_movements(self, session: AsyncSession, days: int = 7, *, skip: int = 0, limit: int = 100) -> List[StockMovement]:
        """
    ...
    async def get_product_movement_history(self, session: AsyncSession, product_id: int, days: int = 30) -> List[StockMovement]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting product movement history for product {product_id}: {e}")
            return []
    
    async def get_daily_movement_summary(self, session: AsyncSession, date: datetime) -> dict:
        """
        ...
        try:
            ...
            breakdown = {row.movement_type: {'count': row.count, 'total_quantity': row.total_quantity} for row in result_breakdown}
    ...
    async def create_in_movement(self, session: AsyncSession, product_id: int, quantity_change: int, 
                                 reference_type: str, reference_id: Optional[int], created_by: int,
                                 movement_date: Optional[datetime] = None) -> Optional[StockMovement]:
        """
        ...
        try:
            ...
            # Update inventory
            await inventory_repo.update_quantity(session, product_id, quantity_change, commit=False)
    ...
    async def create_out_movement(self, session: AsyncSession, product_id: int, quantity_change: int,
                                  reference_type: str, reference_id: Optional[int], created_by: int,
                                  movement_date: Optional[datetime] = None) -> Optional[StockMovement]:
        """
        ...
        try:
            ...
            quantity_before = inventory.quantity_on_hand
            quantity_after = quantity_before + quantity_change  # quantity_change is negative
            
            if quantity_after < 0:
                raise ValueError(f"Insufficient stock for product {product_id}. Available: {quantity_before}, Requested: {-quantity_change}")
