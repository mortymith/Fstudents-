...
class StockAdjustmentRepository:
    ...
    async def get_all(self, session: AsyncSession, *, skip: int = 0, limit: int = 100) -> List[StockAdjustment]:
        """
    ...
    async def update(self, session: AsyncSession, id: Any, **kwargs) -> Optional[StockAdjustment]:
        """
    ...
    async def get_by_product_id(self, session: AsyncSession, product_id: int) -> Optional[StockAdjustment]:
        """
        ...
        """
        try:
            stmt = select(StockAdjustment).where(StockAdjustment.product_id == product_id)
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error getting stock adjustment by product ID {product_id}: {e}")
            return None
    
    async def filter_by_product(self, session: AsyncSession, product_id: int, *, skip: int = 0, limit: int = 100) -> List[StockAdjustment]:
        """
        ...
        try:
            ...
            logger.error(f"Error filtering stock adjustments by product {product_id}: {e}")
            return []
    
    async def filter_by_adjustment_type(self, session: AsyncSession, adjustment_type: str, *, skip: int = 0, limit: int = 100) -> List[StockAdjustment]:
        """
        ...
        """
        try:
            stmt = select(StockAdjustment).where(StockAdjustment.adjustment_type == adjustment_type).offset(skip).limit(limit)
            result = await session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
            logger.error(f"Error filtering stock adjustments by type {adjustment_type}: {e}")
            return []
    
    async def filter_by_creator(self, session: AsyncSession, created_by: int, *, skip: int = 0, limit: int = 100) -> List[StockAdjustment]:
        """
        ...
        """
        try:
            stmt = select(StockAdjustment).where(StockAdjustment.created_by == created_by).offset(skip).limit(limit)
            result = await session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
            logger.error(f"Error filtering stock adjustments by creator {created_by}: {e}")
            return []
    
    async def filter_by_date_range(self, session: AsyncSession, start_date: datetime, end_date: datetime, *, skip: int = 0, limit: int = 100) -> List[StockAdjustment]:
        """
        ...
        try:
            stmt = (
                select(StockAdjustment)
                .where(and_(StockAdjustment.adjustment_date >= start_date, StockAdjustment.adjustment_date <= end_date))
                .order_by(desc(StockAdjustment.adjustment_date))
                .offset(skip)
                .limit(limit)
            )
    ...
    async def filter_by_quantity_adjusted(self, session: AsyncSession, min_quantity: int, max_quantity: Optional[int] = None, *, skip: int = 0, limit: int = 100) -> List[StockAdjustment]:
        """
        ...
        try:
            ...
            if max_quantity is not None:
                conditions.append(func.abs(StockAdjustment.quantity_adjusted) <= max_quantity)
            
            stmt = select(StockAdjustment).where(and_(*conditions)).offset(skip).limit(limit)
            result = await session.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
    ...
    async def get_or_create(self, session: AsyncSession, defaults: Optional[dict] = None, **kwargs) -> Tuple[StockAdjustment, bool]:
        """
    ...
    async def update_or_create(self, session: AsyncSession, criteria: dict, updates: dict) -> Tuple[StockAdjustment, bool]:
        """
        ...
        try:
            ...
            conditions = [getattr(StockAdjustment, key) == value for key, value in criteria.items()]
    ...
    async def get_with_product(self, session: AsyncSession, id: Any) -> Optional[StockAdjustment]:
        """
        ...
        """
        try:
            stmt = select(StockAdjustment).options(selectinload(StockAdjustment.product)).where(StockAdjustment.id == id)
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
        except Exception as e:
    ...
    async def get_with_creator(self, session: AsyncSession, id: Any) -> Optional[StockAdjustment]:
        """
        ...
        """
        try:
            stmt = select(StockAdjustment).options(selectinload(StockAdjustment.creator)).where(StockAdjustment.id == id)
            result = await session.execute(stmt)
            return result.scalar_one_or_none()
        except Exception as e:
    ...
    async def get_recent_adjustments(self, session: AsyncSession, days: int = 7, *, skip: int = 0, limit: int = 100) -> List[StockAdjustment]:
        """
    ...
    async def get_product_adjustment_history(self, session: AsyncSession, product_id: int, days: int = 30) -> List[StockAdjustment]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting product adjustment history for product {product_id}: {e}")
            return []
    
    async def create_damaged_adjustment(self, session: AsyncSession, product_id: int, quantity_adjusted: int, 
                                        reason: str, created_by: int) -> Optional[StockAdjustment]:
        """
        ...
        try:
            ...
            logger.error(f"Error creating damaged adjustment for product {product_id}: {e}")
            return None
    
    async def create_expired_adjustment(self, session: AsyncSession, product_id: int, quantity_adjusted: int, 
                                        reason: str, created_by: int) -> Optional[StockAdjustment]:
        """
        ...
        try:
            ...
            logger.error(f"Error creating expired adjustment for product {product_id}: {e}")
            return None
    
    async def create_returned_adjustment(self, session: AsyncSession, product_id: int, quantity_adjusted: int, 
                                         reason: str, created_by: int) -> Optional[StockAdjustment]:
        """
        ...
        try:
            ...
            logger.error(f"Error creating returned adjustment for product {product_id}: {e}")
            return None
    
    async def create_found_adjustment(self, session: AsyncSession, product_id: int, quantity_adjusted: int, 
                                      reason: str, created_by: int) -> Optional[StockAdjustment]:
        """
        ...
        try:
            ...
            logger.error(f"Error creating found adjustment for product {product_id}: {e}")
            return None
    
    async def _create_adjustment_with_inventory_update(self, session: AsyncSession, product_id: int, 
                                                       quantity_adjusted: int, adjustment_type: str, 
                                                       reason: str, created_by: int) -> Optional[StockAdjustment]:
        """
        ...
        """
        try:
            from app.repositories.product_inventory_repository import ProductInventoryRepository
            ...
            if quantity_adjusted < 0 and inventory.quantity_on_hand + quantity_adjusted < 0:
                raise ValueError(f"Insufficient stock for adjustment. Available: {inventory.quantity_on_hand}, Adjustment: {quantity_adjusted}")
            ...
            # Update inventory
            await inventory_repo.update_quantity(session, product_id, quantity_adjusted, commit=False)
    ...
    async def get_adjustment_summary_by_type(self, session: AsyncSession, start_date: datetime, end_date: datetime) -> dict:
        """
    ...
    async def get_total_loss_value(self, session: AsyncSession, start_date: datetime, end_date: datetime) -> dict:
        """
        ...
        try:
            ...
            stmt = select(
                StockAdjustment.adjustment_type,
                func.sum(StockAdjustment.quantity_adjusted * Product.cost_price).label('total_loss_value')
            ).join(
                Product, StockAdjustment.product_id == Product.id
            ).where(
                and_(
                    StockAdjustment.adjustment_date >= start_date,
                    StockAdjustment.adjustment_date <= end_date,
                    StockAdjustment.adjustment_type.in_(['damaged', 'expired', 'theft']),
